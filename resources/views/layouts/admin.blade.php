...
<head>
    ...
    <title>@yield('title', 'Admin') — ThreatWatch Nigeria</title>
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
            ...
            <a href="{{ route('admin.incidents.index') }}"
               class="sidebar-link {{ request()->routeIs('admin.incidents.*') ? 'active' : '' }}">
                ...
                Incidents
                @php $pendingCount = \App\Models\Incident::pending()->count(); @endphp
                @if($pendingCount)
            ...
            <a href="{{ route('admin.helplines.index') }}"
               class="sidebar-link {{ request()->routeIs('admin.helplines.*') ? 'active' : '' }}">
        ...
        <div class="px-3 py-3 border-t border-gray-100 flex-shrink-0">
            <div class="flex items-center gap-2.5">
                ...
                <div class="min-w-0 flex-1">
                    <p class="text-xs font-bold text-gray-900 truncate">{{ auth()->user()->full_name }}</p>
                    <p class="text-[10px] text-gray-400 capitalize">{{ auth()->user()->role_label }}</p>
                </div>
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header class="h-16 bg-white border-b border-gray-100 flex items-center px-4 sm:px-6 gap-4 flex-shrink-0 z-10">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-gray-400 mt-0.5">@yield('page_breadcrumb')</p>
                @endif
            </div>

            <div class="flex items-center gap-3">
                {{-- Date --}}
                <span class="hidden sm:block text-xs text-gray-400">{{ now()->format('D, d M Y') }}</span>

                {{-- Notifications badge --}}
                @php $newPending = \App\Models\Incident::pending()->count() + \App\Models\Report::pending()->count(); @endphp
                @if($newPending > 0)
                <div class="relative">
