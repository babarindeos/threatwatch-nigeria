...
@section('title', 'ThreatWatch Nigeria — Real-Time Security Incident Tracking')
@section('meta_description', 'Monitor and report security incidents across all 36 states of Nigeria. Real-time banditry, terrorism, kidnapping and armed robbery tracking.')
        ...
        <div class="max-w-3xl">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div class="bg-white/[.06] border border-white/10 rounded-xl px-4 py-3 backdrop-blur-sm">
                    <div class="text-lg font-display font-bold text-white">{{ $s['icon'] }} {{ $s['value'] }}</div>
...
<section class="bg-ng-muted py-14">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            ...
            <a href="{{ route('incidents.index') }}?attack_type={{ $cat['type'] }}"
               class="bg-white rounded-xl p-4 text-center border border-gray-100 hover:border-ng-green/40
                ...
                @if(isset($attackTypeStats[$cat['type']]))
                <p class="text-[10px] text-gray-400 mt-1">{{ $attackTypeStats[$cat['type']]->total }} cases</p>
                @endif
            </a>
            @endforeach
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
        ...
        <div>
            ...
            <div class="space-y-3">
                ...
                @php $maxCount = $topStates->first()->incident_count ?: 1; @endphp
                <div class="flex items-center gap-4">
                    <span class="w-6 text-sm font-bold text-gray-400 text-right flex-shrink-0">{{ $index + 1 }}</span>
                    <div class="flex-1">
                        <div class="flex items-center justify-between mb-1.5">
                            <a href="{{ route('incidents.index') }}?state_id={{ $state->id }}"
                               class="text-sm font-semibold text-gray-800 hover:text-ng-green transition-colors">
                                {{ $state->name }}
                            </a>
                            <span class="text-xs font-bold text-gray-500">{{ $state->incident_count }}</span>
                        </div>
                        <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                            <div class="h-full bg-ng-green rounded-full transition-all duration-700"
                                 style="width: {{ ($state->incident_count / $maxCount) * 100 }}%"></div>
                        </div>
                    </div>
                </div>
                @endforeach
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-8">
            ...
            <div class="text-center">
                ...
                <h3 class="font-display font-bold text-white text-base mb-2">{{ $step['title'] }}</h3>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div class="flex flex-wrap gap-3">
                @foreach(['🚔 Police: 199','🔥 Fire: 01-272-0892','🚑 Ambulance: 0700-2625226','🛡️ DSS: 08057000001'] as $line)
                <div class="bg-white/20 border border-white/30 backdrop-blur-sm text-white
                            text-xs font-bold px-4 py-2.5 rounded-xl">{{ $line }}</div>
                @endforeach
            </div>
